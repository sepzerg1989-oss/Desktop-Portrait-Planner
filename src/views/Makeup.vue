<template>
  <div class="h-full overflow-y-auto px-12 py-10 relative select-none font-sans bg-morandi-canvas text-morandi-text">
    <div class="flex flex-col md:flex-row justify-between items-stretch md:items-end gap-6 mb-12 border-b border-morandi-border/20 pb-6">
      <div>
        <h1 class="text-4xl font-serif text-morandi-text/90 mb-2 tracking-[0.1em]">妆面造型库</h1>
        <p class="text-morandi-muted text-[10px] tracking-[0.3em] uppercase">Makeup Library / Styling</p>
      </div>
    ...
    <transition name="fade">
      <div v-if="showNamePrompt" class="fixed inset-0 z-[60] bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-morandi-paper p-8 shadow-2xl w-[400px] border border-morandi-border rounded-none">
          <h3 class="text-xl font-serif text-morandi-text mb-2">新建妆容造型</h3>
          <p class="text-[10px] uppercase tracking-widest text-morandi-muted mb-6">Create New Makeup</p>
          <div class="mb-8">
            <label class="block text-xs uppercase tracking-wider text-morandi-muted mb-2">输入妆容名称</label>
            <input 
              v-model="promptName" 
              type="text" 
