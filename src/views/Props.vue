  ...
  <div class="h-full overflow-y-auto px-12 py-10 relative select-none font-sans bg-morandi-canvas text-morandi-text">
    ...
    <div v-else class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-8">
      ...
      >
        ...
        <!-- 1:1 比例正方形图区 (满宽贴边) -->
        <div class="aspect-square overflow-hidden bg-morandi-canvas/10 border-b border-morandi-border/10">
          <img v-if="prop.coverURL" :src="prop.coverURL" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
        ...
        <div class="p-4 pb-6 space-y-1.5 text-left">
          ...
          <h2 class="font-serif text-[13px] text-morandi-text font-bold group-hover:text-morandi-red transition-colors duration-300 truncate">
            {{ prop.name }}
            <span v-if="prop.tags && prop.tags.length > 0" class="font-sans font-normal text-[9px] text-morandi-muted/80 tracking-wider ml-1">
              / {{ prop.tags.join(' · ') }}
            </span>
          </h2>

          <!-- 第二行：描述 -->
          <p v-if="prop.description" class="text-[10px] text-morandi-muted/80 truncate font-sans leading-relaxed">
            {{ prop.description }}
          </p>
        </div>
      </div>
