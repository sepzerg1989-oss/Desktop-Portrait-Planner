<template>
  <div class="space-y-10">
    ...
    <div>
      ...
      <div v-if="location?.tags && location.tags.length > 0" class="text-[10px] text-morandi-muted uppercase tracking-[0.2em] mt-2 mb-6 select-none">
        <span v-for="(tag, index) in location.tags" :key="tag">
          {{ tag }}<span v-if="index < location.tags.length - 1" class="mx-2">·</span>
        </span>
      </div>
