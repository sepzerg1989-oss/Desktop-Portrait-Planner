        ...
        >
          ...
          <div class="flex-1">
            ...
            <div class="text-[10px] text-morandi-muted font-sans mt-0.5 truncate">
              ...
              <span v-if="loc.tags && loc.tags.length > 0" class="text-morandi-muted">· {{ loc.tags.slice(0, 2).join(', ') }}</span>
            </div>
          </div>
...
const filteredLocations = computed(() => {
  return locationStore.locations.filter(loc => {
    const matchesSearch = !searchQuery.value.trim() || 
      ...
      (loc.tags && loc.tags.some(t => t.toLowerCase().includes(searchQuery.value.toLowerCase())))
    ...
    const matchesTag = selectedTags.value.length === 0 || 
      (loc.tags && selectedTags.value.some(tag => loc.tags.includes(tag)))
