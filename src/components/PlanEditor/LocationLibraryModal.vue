    ...
    <div v-if="show" class="absolute inset-0 z-[60] bg-white flex flex-col">
      <div class="p-6 border-b border-black/5 flex justify-between items-center bg-morandi-canvas/30">
        <div>
          ...
          <p class="text-[10px] text-morandi-muted uppercase tracking-widest">Select from location library</p>
        </div>
      ...
      <div class="flex-1 overflow-y-auto p-4 space-y-2">
        ...
        >
          ...
          <div class="flex-1">
            ...
            <div class="text-[10px] text-morandi-muted">{{ loc.address || '未知地址' }}</div>
          </div>
          <div class="opacity-0 group-hover:opacity-100 text-morandi-blue text-[10px] uppercase tracking-widest">选择</div>
        </div>
        <div v-if="locationStore.locations.length === 0" class="py-10 text-center text-xs text-morandi-muted">
          场地库暂无数据
        </div>
      </div>
    </div>
