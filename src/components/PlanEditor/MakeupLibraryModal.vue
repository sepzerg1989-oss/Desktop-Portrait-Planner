  ...
  <transition name="fade">
    <div v-if="show" class="absolute inset-0 z-[60] bg-morandi-panel flex flex-col text-morandi-text">
      ...
      <div class="p-6 border-b border-morandi-border flex justify-between items-center bg-morandi-canvas/30 text-morandi-text">
        <div>
          <h3 class="font-serif text-lg">妆容库选择</h3>
          <p class="text-[10px] text-morandi-muted uppercase tracking-widest">Select from makeup library</p>
        </div>
      ...
      <div class="flex-1 overflow-y-auto p-4 space-y-2 scroll-thin">
        ...
        <div v-if="filteredMakeups.length === 0" class="py-10 text-center text-xs text-morandi-muted font-sans">
          暂无匹配的妆容数据
        </div>
      </div>
    </div>
...
const filteredMakeups = computed(() => {
  return makeupStore.makeups.filter(m => {
...
onMounted(async () => {
  ...
  for (const item of makeupStore.makeups) {
