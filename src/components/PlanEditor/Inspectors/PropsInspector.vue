<template>
  <div>
    ...
    <div class="space-y-4">
      ...
      >
        ...
        <div class="flex items-center justify-between pb-3 border-b border-morandi-border/20 cursor-pointer select-none" @click="toggleCollapse(item.id)">
          <div class="flex items-center space-x-2 min-w-0 pr-2">
            ...
            <span class="text-xs font-serif font-bold text-morandi-text truncate">{{ item.name || '未命名拍摄道具' }}</span>
          </div>
        ...
        <transition name="fade-height">
          <div v-show="activeCollapsedId === item.id" class="pt-4 space-y-4">
            <div>
              <label class="block text-[9px] uppercase tracking-wider text-morandi-muted mb-1 font-bold">道具名称 / NAME</label>
              <input v-model="item.name" type="text" class="w-full px-1 py-2 border-b border-morandi-border bg-transparent focus:border-morandi-text outline-none text-xs text-morandi-text rounded-none" placeholder="如: 装饰放大镜" />
            </div>

            <div>
              <label class="block text-[9px] uppercase tracking-wider text-morandi-muted mb-1 font-bold">道具描述 / DESCRIPTION</label>
              <textarea v-model="item.description" rows="2" class="w-full px-1 py-2 border-b border-morandi-border bg-transparent focus:border-morandi-text outline-none text-xs text-morandi-text rounded-none resize-none font-sans" placeholder="如: 材质或摆设要求..."></textarea>
            </div>

            <div>
              <label class="block text-[9px] uppercase tracking-wider text-morandi-muted mb-1 font-bold">分类标签 / TAGS</label>
              <input v-model="item.tagsInput" type="text" placeholder="用逗号分隔，如：古典, 置景" class="w-full px-1 py-2 border-b border-morandi-border bg-transparent focus:border-morandi-text outline-none text-xs text-morandi-text rounded-none" />
            </div>

            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="block text-[9px] uppercase tracking-wider text-morandi-muted mb-1 font-bold">价格/租金 / PRICE</label>
                <input v-model="item.price" type="text" class="w-full px-1 py-2 border-b border-morandi-border bg-transparent focus:border-morandi-text outline-none text-xs text-morandi-text rounded-none" placeholder="￥100/天" />
              </div>
              <div>
                <label class="block text-[9px] uppercase tracking-wider text-morandi-muted mb-1 font-bold">来源链接 / LINK</label>
            ...
            <div>
              <label class="block text-[9px] uppercase tracking-wider text-morandi-muted mb-1 font-bold font-serif">道具照片 / PHOTOS</label>
              <ImageUploader 
                v-model="item.images" 
      ...
      >
        <span class="text-[9px] font-sans text-morandi-muted uppercase tracking-widest block mb-1">No Props Available</span>
        <span class="text-[11px] font-serif italic text-morandi-muted/80">* 暂无拍摄道具，请点击上方按钮添加道具或从素材库导入</span>
      </div>
    </div>
  </div>
</template>
