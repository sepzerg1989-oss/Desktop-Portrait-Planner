  ...
  <transition name="fade">
    <div v-if="show" class="absolute inset-0 z-[60] bg-morandi-panel flex flex-col text-morandi-text">
      ...
      <div class="p-6 border-b border-morandi-border flex justify-between items-center bg-morandi-canvas/30 text-morandi-text">
        <div>
          <h3 class="font-serif text-lg">服装库选择</h3>
          <p class="text-[10px] text-morandi-muted uppercase tracking-widest">Select from clothing library</p>
        </div>
      ...
      <div class="flex-1 overflow-y-auto p-4 space-y-2 scroll-thin">
        <div 
          v-for="item in filteredClothings" 
          :key="item.id"
        ...
        <div v-if="filteredClothings.length === 0" class="py-10 text-center text-xs text-morandi-muted font-sans">
          暂无匹配的服装数据
        </div>
      </div>
    </div>
...
const topTags = computed(() => {
  const all = clothingStore.clothings.flatMap(c => c.tags || []).filter(Boolean)
  const counts = {}
...
const filteredClothings = computed(() => {
  return clothingStore.clothings.filter(c => {
    ...
    const matchesTag = selectedTags.value.length === 0 || 
      (c.tags && selectedTags.value.some(tag => c.tags.includes(tag)))
...
onMounted(async () => {
  ...
  // 同步加载首图映射
  for (const item of clothingStore.clothings) {
