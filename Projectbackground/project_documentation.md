...
- **核心逻辑**：基于“工作区 (Workspace)”概念实现数据与图片的本地闭环管理。用户可以管理模特库、场地库，并拼装出结构化、模块化的拍摄策划案，支持拖拽排序、自动计算黄金日照时刻，以及一键导出长图或策划包数据。
- **开发约束**：
  - 代码精简、模块化。
...
- **状态管理**：`Pinia` 提供全局状态存储 (`planStore`, `modelStore`, `locationStore`)。
- **数据引擎**：`better-sqlite3`（SQLite3 的原生 C++ 绑定驱动，提供极高的本地读写并发性能）。
- **样式方案**：`Tailwind CSS` + `PostCSS`（界面设计采用高质感的莫兰迪色系 `#E2DED0`，全无边框设计 `frame: false`）。
- **辅助库**：
  - `sharp`：主进程中用于本地图片的压缩与格式优化，减少图片占用空间。
  - `suncalc`：基于经纬度或行政区自动计算拍摄当日的日出、日落、黄金时间段。
  - `vuedraggable`：前端用于策划案内部模块的拖拽排序。
  - `electron-store`：管理和记忆基础客户端配置（如上次打开的工作区路径）。
...
| | `exportData(ids)` | `system:exportData` | 导出选择的策划案打包数据 |
...
前端采用 Vue-Router 控制页面路由，核心视图均映射至独立的状态 Store。
...
- **策划编辑器 (`/editor` -> `PlanEditor.vue`)**：
  ...
  - **中栏**：编辑表单，自动根据当前激活的模块类型映射对应的表单控件。
  - **右栏**：扩展控制工具（如黄金时刻计算参数、批量编辑栏等）。
...
5. `clothing`（模特服装）：服装描述与穿搭参考图。
6. `props`（拍摄道具）：道具清单、描述与参考图。
7. `shoot_time`（拍摄日期）：结合经纬度或城市，调用日照算法自动渲染“黄金时间”、“蓝调时间”、“日出日落”。
8. `custom`（自定义模块）：通用富文本描述与多图。
...
2. **分析业务诉求**：
   ...
   - 如果用户要**对模特库或场地库添加字段**：
     ...
     - 修改 [modelStore.js](file:///d:/software_dev/New_Portraitplanner_desktop/APP/src/store/modelStore.js) 或 [locationStore.js](file:///d:/software_dev/New_Portraitplanner_desktop/APP/src/store/locationStore.js) 对应的持久化映射。
     - 最后修改 [Models.vue](file:///d:/software_dev/New_Portraitplanner_desktop/APP/src/views/Models.vue) 或 [Locations.vue](file:///d:/software_dev/New_Portraitplanner_desktop/APP/src/views/Locations.vue) 的表单组件。
3. **保持高水准的开发标准**：
